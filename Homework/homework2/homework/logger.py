...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        #logger.add_scalar("train_accuracy", torch.mean(torch.tensor(metrics["train_acc"])), global_step=global_step)
        logger.add_scalar("train_accuracy", torch.mean(torch.cat(metrics["train_acc"])), global_step=global_step)
        ...
        #logger.add_scalar("val_accuracy", torch.mean(torch.tensor(metrics["val_acc"])), global_step=global_step)
        logger.add_scalar("val_accuracy", torch.mean(torch.cat(metrics["val_acc"])), global_step=global_step)
