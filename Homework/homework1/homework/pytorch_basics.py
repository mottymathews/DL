...
class PyTorchBasics:
    ...
    def make_it_pytorch_3(x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torch.unique(x).sort().values
    ...
    def make_it_pytorch_4(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        mean_x = x.mean()
        return (y > mean_x).sum().to(torch.int64)
        raise NotImplementedError
    ...
    def make_it_pytorch_extra_2(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        """
        diff = (x[:, None] - y[None, :]).abs()         # Shape: (len(x), len(y))
        matches = (diff < 1e-3).any(dim=1)             # True if x[i] matches any y[j]
        count = matches.sum()                          # Number of matching elements
        return count
        raise NotImplementedError
